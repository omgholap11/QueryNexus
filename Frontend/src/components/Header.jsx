...
export default function Header({ title }) {
    return (
        <header className="flex items-center justify-between px-8 py-4 sticky top-0 z-10 pl-14 md:pl-8 bg-background-dark/80 backdrop-blur-md">
            <div className="flex-1">
                {title && (
                    <div className="animate-fade-in-up">
                        <p className="text-sm font-medium text-slate-400">Researching</p>
                        <p className="text-base font-bold text-white truncate max-w-md">{title}</p>
                    </div>
                )}
            </div>
            <div className="flex items-center justify-end gap-3 cursor-pointer group">
